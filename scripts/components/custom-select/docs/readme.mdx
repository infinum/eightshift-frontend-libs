...
As this control can be used as a synchronous (options passed to component) or an asynchronous (options loaded via callback) component,
...
| `isSearchable` | _(optional)_ If `true` allows searching through results. Default: `true`. |
