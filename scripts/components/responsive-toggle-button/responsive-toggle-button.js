				...
				return (
					...
					>
						<Button
							...
							icon={
								buttonIcon ??
								React.cloneElement(toggleOff, {
									className: clsx('es-animated-toggle-icon', isActive && 'is-checked'),
								})
							}
							isPressed={buttonToggleEffect && isActive}
							className={clsx('es-button-square-32 es-rounded-1! es-h-8! es-px-2!', buttonToggleEffect ? 'es-is-v2-gutenberg-button es-button-icon-24' : 'es-has-v2-gutenberg-button-active-state es-button-icon-30', buttonToggleEffect && isActive && 'is-active')}
							{...additionalProps}
						/>
					</div>
				);
