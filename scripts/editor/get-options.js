...
export const getOptions = (manifest = {}, componentName, attribute, options = {}) => {
	...
	let originalKey = `${(manifest.componentName)}${pascalCase(attribute).replace(/\s/g, '')}`;
	let customKey = `${camelCase(componentName)}${pascalCase(attribute).replace(/\s/g, '')}`;
	...
	if (Object.prototype.hasOwnProperty.call(options, customKey) && !isEqual(manifest.options[originalKey], options[customKey])) {
