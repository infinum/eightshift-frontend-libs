...
Load more component must be implemented in some other component because it can't work by it self.
It also depends on the PHP part of dynamic data that can be instaled with this command:

```shell
wp boilerplate create_dynamic_data
```
...
1. You have featured content block that must implemnet load more component.
...
5. You must provide load more component `loadMoreType` attribute that will determin what map you will use in you dynamic data class to provide view output.
...
6. Optional load more component `loadMorePerPageOverride` attribute is used to load different number of items than they are initialy set in the DOM.
...
9. In you component you must have partial view component that will map ids from the dynamic data to the actial output.
10. Keep in mind that you also must have `esBlocksLocalization` global variable provided with ajaxurl variable in your `EnqueueTheme` class.
